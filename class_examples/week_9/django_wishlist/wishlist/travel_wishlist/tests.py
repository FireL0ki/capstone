...
class TestHomePage(TestCase):
    ...
    def test_home_page_shows_empty_list_message_for_empty_database(self):
        ...
        response = self.client.get(home_page_url)  #self is the test case / client can make requests to web application server
        self.assertTemplateUsed(response, 'travel_wishlist/wishlist.html')
        self.assertContains(response, 'You have no places in your wishlist')

class TestWishList(TestCase): # new TestCases for different types of tests
...
class TestVisitedPage(TestCase):
    ...
    def test_visited_page_shows_empty_list_when_empty_database(self):
...
class TestVisitPlace(TestCase):
    ...
    def test_non_existent_place(self):
        visit_nonexistent_place_url = reverse('place_was_visited', args=(23456, ))
